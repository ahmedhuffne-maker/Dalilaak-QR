...
export const PreviewPanel: React.FC<PreviewPanelProps> = ({
  ...
  const formats: { id: PosterFormat; label: string; desc: string }[] = [
    { id: 'a4_quad', label: 'A4 شيت (4 نسخ بالورقة)', desc: '210 × 297 مم (4 بطاقات للقص)' },
    { id: 'a3_quad', label: 'A3 شيت (4 نسخ بالورقة)', desc: '297 × 420 مم (4 بطاقات كبيرة للقص)' },
    ...
    { id: 'square', label: 'مربع للطاولات', desc: 'ستيكر 1:1' },
  ...
  return (
    <div className="bg-slate-900/80 backdrop-blur-xl rounded-3xl border border-slate-800/90 shadow-2xl flex flex-col h-full overflow-hidden">
      ...
      <div className="p-3 sm:p-3.5 border-b border-slate-800/80 bg-slate-950/60 flex items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-1 bg-slate-850 p-1 rounded-2xl border border-slate-750 shadow-inner">
          <button
            ...
            className={`flex items-center gap-1.5 px-3.5 py-1.5 rounded-xl text-xs font-bold transition-all cursor-pointer ${
              viewMode === 'canvas'
                ? 'bg-amber-500 text-slate-950 shadow-md shadow-amber-500/20 font-extrabold scale-100'
          ...
          >
            ...
            <span>المعاينة الواقعية (3D Mockup)</span>
          </button>
        </div>
      ...
      {/* Dedicated Formats Bar: Fully visible, wrapping chips without truncation */}
      <div className="px-3.5 py-2.5 bg-slate-950/40 border-b border-slate-800/80 flex flex-col sm:flex-row items-start sm:items-center gap-2.5">
        <div className="flex items-center gap-1.5 text-xs font-bold text-slate-300 shrink-0">
          <SlidersHorizontal className="w-3.5 h-3.5 text-amber-400" />
          <span>مقاس القالب:</span>
        </div>
              ...
              <button
                ...
                className={`px-3 py-1.5 rounded-xl text-xs font-bold transition-all cursor-pointer flex items-center gap-1.5 border ${
                  isSelected
                    ...
                    : 'bg-slate-850/80 hover:bg-slate-800 text-slate-300 border-slate-750 hover:border-slate-650'
                }`}
                title={fmt.desc}
              >
      ...
      {/* Interactive Quick QR Code Upload Bar in Preview Panel */}
      {onChangeConfig && (
        <div className="px-4 py-2.5 bg-slate-950/70 border-b border-slate-800/70 flex items-center justify-between gap-3 text-right">
          {config.uploadedQrDataUrl ? (
            <div className="flex items-center justify-between w-full">
              <div className="flex items-center gap-2.5">
                ...
                <div className="text-right">
                  <span className="text-xs font-bold text-emerald-400 flex items-center gap-1">
                    ...
                    تم دمج رمز الـ QR المخصص في المعاينة
                  </span>
                  <span className="text-[10px] text-slate-400 hidden sm:inline">
                    يتم استبدال الرمز الأصلي بالرمز المرفوع مباشرة وبأعلى جودة
                  </span>
                </div>
              </div>
              ...
              <div className="flex items-center gap-1.5">
                <button
                  ...
                  className="px-2.5 py-1 bg-slate-800 hover:bg-slate-700 text-slate-200 text-xs font-bold rounded-lg border border-slate-700 transition-colors cursor-pointer"
                >
                  استبدال
                </button>
                <button
            ...
            >
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <span className="text-xs font-bold text-slate-200 group-hover:text-amber-300 transition-colors">
                    رفع رمز QR Code لدمجه فوراً في الملصق
                  </span>
                  <span className="text-[11px] text-slate-400 block sm:inline sm:mr-2">
                    (انقر هنا أو اسحب صورة الكود وأفلتها فوق المعاينة)
                  </span>
                </div>
              </div>
              ...
              <button
                type="button"
                className="px-3 py-1 bg-amber-500 hover:bg-amber-400 text-slate-950 text-xs font-extrabold rounded-lg shadow-sm transition-transform group-hover:scale-105 cursor-pointer shrink-0"
              >
                + رفع كود QR
              </button>
            </div>
      ...
      <div 
        ...
        className={`flex-1 bg-slate-950/80 p-6 overflow-auto flex items-center justify-center relative min-h-[480px] transition-colors ${
      ...
      >
        ...
        {/* Loading Overlay */}
        {isRendering && (
          <div className="absolute top-4 right-4 z-20 flex items-center gap-2 bg-slate-900/90 backdrop-blur-md px-3.5 py-1.5 rounded-full border border-amber-500/40 text-amber-400 text-xs font-bold shadow-lg animate-pulse">
            <Sparkles className="w-3.5 h-3.5 animate-spin" />
            <span>جاري التحديث والرسم بدقة فائقة...</span>
          </div>
        )}

        <div 
          className={`transition-transform duration-200 flex items-center justify-center ${viewMode === 'canvas' ? '' : 'hidden'}`}
          style={{ transform: `scale(${zoom / 100})`, transformOrigin: 'center center' }}
        >
          <canvas
            ref={canvasRef}
            id="poster-canvas"
            className="max-h-[620px] max-w-full w-auto object-contain rounded-2xl shadow-[0_25px_60px_-15px_rgba(0,0,0,0.8)] border border-slate-700/60"
          />
        </div>
      ...
      <div className="p-4 border-t border-slate-800/80 bg-slate-950/80 flex flex-col sm:flex-row items-center justify-between gap-3">
        
        {/* Export Quality Note */}
        <div className="flex items-center gap-2 text-xs text-slate-400">
          <span className="w-2 h-2 rounded-full bg-emerald-400" />
          <span>توليد فوري بجودة طباعة 300 DPI جاهزة للمطابع والإعلانات</span>
        </div>
        ...
        <div className="flex items-center gap-2 w-full sm:w-auto justify-end flex-wrap">
          ...
          <button
            ...
            className="flex-1 sm:flex-initial inline-flex items-center justify-center gap-1.5 bg-slate-800/90 hover:bg-slate-750 text-slate-200 text-xs font-bold py-2.5 px-3 rounded-xl border border-slate-700/80 transition-colors cursor-pointer"
          ...
          <button
            ...
            className="flex-1 sm:flex-initial inline-flex items-center justify-center gap-1.5 bg-slate-800/90 hover:bg-slate-750 text-slate-200 text-xs font-bold py-2.5 px-3.5 rounded-xl border border-slate-700/80 transition-colors cursor-pointer"
            title="تصدير ملف PDF للطباعة"
          >
            <FileText className="w-3.5 h-3.5 text-rose-400" />
            <span>ملف PDF للطباعة</span>
          </button>
          ...
          <button
            ...
            className="flex-1 sm:flex-initial inline-flex items-center justify-center gap-2 bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-400 hover:to-amber-500 text-slate-950 text-xs font-extrabold py-2.5 px-4 rounded-xl shadow-lg shadow-amber-500/20 transition-all cursor-pointer active:scale-95"
          >
            <Download className="w-4 h-4 stroke-[2.5]" />
            <span>تحميل بدقة فائقة (PNG)</span>
          </button>
